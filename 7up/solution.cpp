...
const string heart[] = {"AH", "2H", "3H", "4H", "5H", "6H", "7H", "8H", "9H", "10H", "JH", "QH", "KH"};
const string spade[] = {"AS", "2S", "3S", "4S", "5S", "6S", "7S", "8S", "9S", "10S", "JS", "QS", "KS"};
const string club[] = {"AC", "2C", "3C", "4C", "5C", "6C", "7C", "8C", "9C", "10C", "JC", "QC", "KC"};
const string diamond[] = {"AD", "2D", "3D", "4D", "5D", "6D", "7D", "8D", "9D", "10D", "JD", "QD", "KD"};

bool coba(const string* arr, const string& s) {
	auto it = find(arr, arr + 13, s);
	bool first = 1;
	if (it != arr + 13) {
		int pos = distance(arr, it);
		FORD (i, pos - 1, 0) {
			if (first) first = 0;
			else cout << " ";
			cout << arr[i];
...
int main(int argc, char** argv) {
	...
	FOR (i, 0, n) {
		cin >> s;
		if (coba(heart, s) || coba(spade, s) || coba(club, s) || coba(diamond, s));
		else assert(false);
	}
	return 0;
}
